...
private:
	...
	boost::asio::basic_waitable_timer<std::chrono::steady_clock>
		request_deadline_{acceptor_.get_executor().context(),
	...
	boost::optional<http::response_serializer<http::string_body,
											  http::basic_fields<alloc_t>>>
		string_serializer_{};
	...
	void process_request(
		const http::request<request_body_t, http::basic_fields<alloc_t>> &req) {
		switch (req.method()) {
		case http::verb::get: send_file(req.target()); break;
	...
	void check_deadline() {
		...
		request_deadline_.async_wait(
			[this](boost::beast::error_code) { check_deadline(); });
	}
};
